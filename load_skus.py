        ...
        with app.app_context():
            ...
            for index, row in df.iterrows():
                ...
                category = Category.query.filter_by(
                    name=row["Product Line"], brand_id=brand.id
                ).first()
                if not category:
                ...
                subcategory = Subcategory.query.filter_by(
                    name=row["Product Type"], category_id=category.id
                ).first()
                if not subcategory:
                ...
                if existing_sku:
                    print(
                        f"SKU with material {row['Material']} already exists. Skipping duplicate."
                    )
                    continue
